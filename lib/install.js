...
const { ensureDir } = require('./util/fs.js');
...
function place_binary(uri, targetDir, opts, callback) {
	(async () => {
		...
		const res = await fetch(sanitized, requestOpts);
		if (!res.ok) {
...
function install(gyp, argv, callback) {
	...
	existsAsync(binary_module, (found) => {
		if (!update_binary) {
			if (found) {
				log.info(`[${package_json.name}] Success: "${binary_module}" already installed`);
		...
		function after_place(err) {
			...
			log.info(`[${package_json.name}] Success: "${binary_module}" is installed via remote`);
			return callback();
		}
	});
}
